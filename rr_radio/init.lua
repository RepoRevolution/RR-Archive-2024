...
if not success then return LOG(4, message) end
if not lib then return LOG(4, ('`%s` needs `ox_lib` resource to run with version 3.0.0+'):format(RESOURCE)) end

success, message = lib.checkDependency('es_extended', '1.10.0')
if not success then return LOG(4, message) end
ESX = exports['es_extended']:getSharedObject()
if not ESX then return LOG(4, ('`%s` needs `es_extended` framework resource to run with version 1.10.0+'):format(RESOURCE)) end
