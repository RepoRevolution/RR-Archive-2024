...
AddEventHandler('playerEnteredScope', function(data)
    ...
    if not scopes[player] then
        ...
        ESX.Print('warning', ('Player Id ^3%s^7 did not have its scope configured beforehand. However that is handled, but it should not have happened!'):format(player))
    end
...
function ESX.TriggerScopedEvent(eventName, scopeOwner, includeScopeOwner, ...)
    ...
    if not targets then return ESX.Print('error', ('No such scope (^5%s^7) is available!'):format(scopeOwner)) end
