...
SetConvarReplicated('inventory:weight', Config.Player.maxWeight * 1000) ---@diagnostic disable-line: param-type-mismatch
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer, isNew)
    if not isNew or not Config.Player.starting_accounts.money then return end

    Citizen.Wait(1000)

    if not ESX.Players[source] then return end

    xPlayer.setMoney(Config.Player.starting_accounts.money)
end)
