...
AddStateBagChangeHandler(nil, 'global', function(bn, key, value, _, _)
    ...
    if string.sub(key, 1, notEventLength) ~= notEvent then
        return ESX.Print('error', ('Mulfunctioned GlobalState bag ^4name^7 (^2%s^7) received for triggering safe event!'):format(key))
    end

    local eventName = string.sub(key, notEventLength + 1)

    if not registeredEvents[eventName] then
        return ESX.Print('error', ('The event (^3%s^7) passed in ^4ESX.TriggerSafeEvent^7 is not registered on server!'):format(eventName))
    end

    registeredEvents[eventName].callback(value)
end)

-- local function stateBagChangeHandler(bagName, key, value, _, _)
--     print(bagName, key, value)
--     if type(value) ~= 'table' then return end

--     local bagName = string.match(key, '(.-)%s*->')
--     local playerId = bagName and tonumber(bagName:gsub('player:', ''), 10)
...
--     if string.sub(key, 1, notEventLength) ~= notEvent then
--         return ESX.Print('error', ('Mulfunctioned PlayerState bag ^4name^7 (^2%s^7) received for triggering safe server event!'):format(key))
--     end

--     local eventName = string.sub(key, notEventLength + 1)

--     if not registeredEvents[eventName] then
--         return ESX.Print('error', ('The event (^3%s^7) passed in ^4ESX.TriggerSafeServerEvent^7 is not registered on server!'):format(eventName))
--     end
...
---@diagnostic disable-next-line: param-type-mismatch
AddStateBagChangeHandler('player:1->testowyEvent', nil, function(bn, key, value, _, _)
    print(bn, key, value)
end)

local function addStateBagsForAll()
    
end

local function removeStateBagsForAll()
...
function ESX.TriggerSafeEvent(eventName, source, eventData, eventOptions)
    ...
    if type(eventName) ~= 'string' then
        return ESX.Print('error', ('The event (^3%s^7) passed in ^4ESX.TriggerSafeEvent^7 is not a valid string!'):format(eventName))
    end

    source = tonumber(source) --[[@as number]]
    source = source and math.floor(source)

    if not source or (source <= 0 and source ~= -1) then return ESX.Print('error', 'The source passed in ^4ESX.TriggerSafeEvent^7 must be a valid player id or -1!') end

    if eventData and type(eventData) ~= 'table' then
        return ESX.Print('error', ('The data (^3%s^7) passed in ^4ESX.TriggerSafeEvent^7 is not a table type!'):format(eventName))
    end
