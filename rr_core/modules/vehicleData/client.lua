...
local function loadModel(vehicleModel)
    ...
    ESX.Trace('Trying to load vehicle model ' .. vehicleModel) ---@diagnostic disable-line: param-type-mismatch
    ...
    if not hasModelLoaded then
        return false, ESX.Print('warning', ('^1Waited %s-frames for vehicle model ^5%s ^1to load, but it did not!^7'):format(MAX_TRY_COUNT, vehicleModel))
    end
...
local function spawnPreviewVehicle(vehicleModel, atCoords)
    ...
    while not DoesEntityExist(vehicleEntity) do Citizen.Wait(0) end
...
ESX.RegisterClientCallback('esx:generateVehicleData', function(params)
    ...
    freezeEntity(true, cache.ped, coords)
    ...
    for i = 1, numModels do
        ...
        if isThisModelSpecified or params?.processAll or (not specifiedModel and not params?.processAll and not ESX.GetVehicleData(model)) then
            ...
            if hash then
                local vehicle = spawnPreviewVehicle(hash, coords)

                freezeEntity(true, vehicle, coords)
                ...
                if isThisModelSpecified and not params.processAll then
                    break
                end
    ...
    ESX.HideUI()
    ESX.Trace(message:format(numParsed))
