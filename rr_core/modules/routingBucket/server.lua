        ...
        entities = setmetatable({}, {
            ...
            __newindex = function(self, index, value)
                ...
                if value then
                    ...
                    if Server.Vehicles[index] then Server.Vehicles[index].routingBucket = bucketId end
...
local function onPlayerDropped(source)
    ...
    if not bucketId then return ESX.Print('warning', ('Player Id (^5%s^7) surprisingly did not have routing bucket id assigned!'):format(source)) end
...
do
    exports.rr_core:registerHook('onPlayerLoad', function(payload)
        ...
        if not xPlayer then return ESX.Print('error', 'Unexpected behavior from onPlayerLoad hook in modules/routingBucket/server.lua') end
    ...
    exports.rr_core:registerHook('onVehicleCreate', function(payload)
        local xVehicle = payload?.xVehicle and Server.Vehicles[payload.xVehicle?.entity]

        if not xVehicle then return ESX.Print('error', 'Unexpected behavior from onVehicleCreate hook in modules/routingBucket/server.lua') end
