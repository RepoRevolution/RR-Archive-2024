...
function ESX.OneSync.SpawnVehicle(model, coords, heading, properties, cb)
    ...
    if typeModel ~= 'string' and typeModel ~= 'number' then
        ESX.Print('error', ('Invalid type of model (^1%s^7) in ^5ESX.OneSync.SpawnVehicle^7!'):format(typeModel))
        return
    end
    ...
    local entity = Server.SpawnVehicle(model, modelData.type, coords, heading or coords.w or coords.heading or 0.0)
...
function ESX.OneSync.SpawnObject(model, coords, heading, cb)
    ...
    if not doesEntityExist then return ESX.Print('warning', ('Spawning (^3%s^7) timed out after waiting %s ticks for object creation!'):format(entity, timeout)) end
...
function ESX.OneSync.SpawnPed(model, coords, heading, cb)
    ...
    if not doesEntityExist then return ESX.Print('warning', ('Spawning (^3%s^7) timed out after waiting %s ticks for ped creation!'):format(entity, timeout)) end
...
function ESX.OneSync.SpawnPedInVehicle(model, vehicle, seat, cb)
    ...
    if not doesEntityExist then return ESX.Print('warning', ('Spawning ped in vehicle (^3%s^7) timed out after waiting %s ticks for ped creation!'):format(entity, timeout)) end
...
function ESX.OneSync.MakeVehicleEmptyOfPassengers(vehicleEntity, seatsToEmpty)
    ...
    if not maxNoSeats then
        ESX.Print('error', ('Vehicle hash (^1%s^7) is invalid \nEnsure vehicle exists in ^2`@es_extended/files/vehicles.json`^7'):format(vehicleHash))
        return false
    end
