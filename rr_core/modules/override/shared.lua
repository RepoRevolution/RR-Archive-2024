...
function ESX.SetField(fieldName, value)
    ...
    if fieldNameType ~= 'string' then
        ESX.Print('error', ('The field name (^3%s^7) passed in ^5ESX.SetField^7 in ^3%s^7 is not a valid string!'):format(fieldName, lib.context))
        return false
    elseif not isValueValid then
        ESX.Print('error', ('The value passed in ^5ESX.SetField^7 in ^3%s^7 does not have a valid type!'):format(lib.context))
        return false
    end

    if fieldName == 'SetField' then
        ESX.Print('error', ('Field ^2%s^7 of ESX ^1cannot^7 be overrided!'):format(fieldName))
        return false
    end

    ESX[fieldName] = value

    ESX.Trace(('Setting field (^2%s^7) for ^2ESX^7 through ^5ESX.SetField^7 in ^3%s^7.'):format(fieldName, lib.context))
...
function ESX.SetFunction(fnName, fn)
    ...
    if fnNameType ~= 'string' then
        ESX.Print('error', ('The function name (^3%s^7) passed in ^5ESX.SetFunction^7 in ^3%s^7 is not a valid string!'):format(fnName, lib.context))
        return false
    elseif not isFnValid then
        ESX.Print('error', ('The function passed in ^5ESX.SetFunction^7 in ^3%s^7 is not a valid function!'):format(lib.context))
        return false
    end

    if fnName == 'SetFunction' then
        ESX.Print('error', ('Function ^2%s^7 of ESX ^1cannot^7 be overrided!'):format(fnName))
        return false
    end
    ...
    ESX.Trace(('Setting function (^2%s^7) for ^2ESX^7 through ^5ESX.SetFunction^7 in ^3%s^7.'):format(fnName, lib.context))
