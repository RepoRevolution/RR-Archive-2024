...
function ESX.AddJob(jobObject)
    ...
    for index in pairs(jobsTable) do
        ESX.Trace(('Job ^5`%s`^7 (%s) has been added'):format(jobsTable[index].label, jobsTable[index].name))
    end
...
function ESX.UpdateJob(jobObject)
    ...
    for index in pairs(jobsTable) do
        ESX.Trace(('Job ^5`%s`^7 (%s) has been updated'):format(jobsTable[index].label, jobsTable[index].name))
    end
...
function ESX.RemoveJob(jobObject)
    ...
    for index in pairs(jobsTable) do
        ESX.Trace(('Job ^5`%s`^7 (%s) has been removed'):format(jobsTable[index].label, jobsTable[index].name))
    end
...
function ESX.RefreshJobs()
    ...
    for _, v in ipairs(jobGrades) do
        if Jobs[v.job_name] then
            ...
            ESX.Print('warning', ('Ignoring job grades for ^5`%s`^0 due to missing job'):format(v.job_name))
        end
    end
    ...
    for _, v in pairs(Jobs) do
        if ESX.Table.SizeOf(v.grades) == 0 then
            Jobs[v.name] = nil
            ESX.Print('warning', ('Ignoring job ^5`%s`^0 due to no job grades found'):format(v.name))
        end
    end
...
function Server.RefreshPlayersJob()
