...
local xPlayerMethods     = {
    ...
    addGroup = function(self)
        ...
        return function(groupName, groupGrade)
            ...
            if not ESX.DoesGroupExist(groupName, groupGrade) then
                ESX.Print('warning', ('Ignoring invalid ^5.addGroup(%s, %s)^7 usage for Player ^5%s^7'):format(groupName, groupGrade, self.source))
                return false
            end
            ...
            if Config.AdminGroupsByName[groupName] or groupName == Server.DefaultGroup then
    ...
    removeGroup = function(self)
        ...
        return function(groupName)
            if type(groupName) ~= 'string' or groupName == Server.DefaultGroup or not self.hasGroup(groupName) then return false end

            local triggerAddGroup, defaultGroup = false, Server.DefaultGroup
            local lastGroups = json.decode(json.encode(self.groups))
    ...
    setGroup = function(self)
        ...
        return function(newGroup)
            if not Config.AdminGroupsByName[newGroup] and Server.DefaultGroup ~= newGroup then
                ESX.Print('warning', ('Ignoring invalid ^5.setGroup(%s)^7 usage for Player ^5%s^7'):format(newGroup, self.source))
                return false
            end
    ...
    setAccountMoney = function(self)
        ...
        return function(accountName, money, reason)
            ...
            if not money or money < 0 then
                ESX.Print('error', ('Tried to set account ^5%s^0 for Player ^5%s^0 to with invalid value -> ^5%s^7'):format(accountName, self.playerId, money))
                return false
            end
            ...
            if not account then
                ESX.Print('error', ('Tried to set money to an invalid account ^5%s^0 for Player ^5%s^0'):format(accountName, self.playerId))
                return false
            end
    ...
    addAccountMoney = function(self)
        ...
        return function(accountName, money, reason)
            ...
            if not money or money <= 0 then
                ESX.Print('error', ('Tried to add account ^5%s^0 for Player ^5%s^0 with an invalid value -> ^5%s^7'):format(accountName, self.playerId, money))
                return false
            end
            ...
            if not account then
                ESX.Print('error', ('Tried to add money to an invalid account ^5%s^0 for Player ^5%s^0'):format(accountName, self.playerId))
                return false
            end
    ...
    removeAccountMoney = function(self)
        ...
        return function(accountName, money, reason)
            ...
            if not money or money <= 0 then
                ESX.Print('error', ('Tried to remove account ^5%s^0 for Player ^5%s^0 with an invalid value -> ^5%s^7'):format(accountName, self.playerId, money))
                return false
            end
            ...
            if not account then
                ESX.Print('error', ('Tried to remove money from an invalid account ^5%s^0 for Player ^5%s^0'):format(accountName, self.playerId))
                return false
            end
    ...
    removeInventoryItem = function(self)
        ...
        return function(itemName, itemCount)
            ...
            if newCount < 0 then
                ESX.Print('error', ('Tried to remove non-existance count(%s) of %s item for Player ^5%s^0'):format(itemCount, itemName, self.playerId))
                return false
            end
    ...
    canCarryItem = function(self)
        ...
        return function(itemName, itemCount)
            if not ESX.Items[itemName] then
                ESX.Print('warning', ('Item ^5`%s`^7 was used but does not exist!'):format(itemName))
                return false
            end
    ...
    setJob = function(self)
        ...
        return function(job, grade, duty)
            if not ESX.DoesJobExist(job, grade) then
                ESX.Print('warning', ('Ignoring invalid ^5.setJob()^7 usage for Player ^5%s^7, Job: ^5%s^7'):format(self.source, job))
                return false
            end
    ...
    getMetadata = function(self)
        ...
        return function(index, subIndex)
            ...
            if type(index) ~= 'string' then
                ESX.Print('error', 'xPlayer.getMetadata ^5index^7 should be ^5string^7!')
            else
    ...
    setMetadata = function(self)
        ...
        return function(index, value, subValue)
            if not index then
                ESX.Print('error','xPlayer.setMetadata ^5index^7 is Missing!')
                return false
            end

            if type(index) ~= 'string' then
                ESX.Print('error','xPlayer.setMetadata ^5index^7 should be ^5string^7!')
                return false
            end
            ...
            if not subValue then
                if not validValueTypes[_type] then
                    ESX.Print('error',('xPlayer.setMetadata ^5%s^7 should be ^5number^7 or ^5string^7 or ^5table^7 or ^5boolean^7!'):format(value))
                    return false
                end
                ...
                if _type ~= 'number' and _type ~= 'string' then
                    ESX.Print('error',('xPlayer.setMetadata ^5value^7 should be ^5string^7 as a subIndex!'):format(value))
                    return false
                end
                ...
                if not validValueTypes[type(subValue)] then
                    ESX.Print('error',('xPlayer.setMetadata ^5%s^7 should be ^5number^7 or ^5string^7 or ^5table^7 or ^5boolean^7!'):format(subValue))
                    return false
                end
    ...
    clearMetadata = function(self)
        ...
        return function(index) -- TODO: Get back to this as it looks like the return value won't work properly with all different cases...
            if not index then
                ESX.Print('error',('xPlayer.clearMetadata ^5%s^7 is missing!'):format(index))
                return false
            end
            ...
            if not self.metadata[index] then
                ESX.Print('error', ('xPlayer.clearMetadata ^5%s^7 not exist!'):format(index))
                return false
            end
