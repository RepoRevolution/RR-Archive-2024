...
function CreateExtendedPlayer(cid, playerId, playerIdentifier, playerGroups, playerGroup, playerAccounts, playerInventory, playerInventoryWeight, playerJob, playerLoadout, playerName, playerMetadata)
    ...
    self.maxWeight = Config.Player.max_weight
    ...
    for fnName, fn in pairs(Server.ExtendedPlayerMethods) do
        self[fnName] = fn(self)
    end
...
function ESX.GetPlayerFromCid(cid)
    return Server.PlayersByCid[cid:upper()]
end

---@param identifier string
---@return xPlayer?
function ESX.GetPlayerFromIdentifier(identifier)
    return Server.PlayersByIdentifier[identifier]
end

---@param xPlayer xPlayer
---@param cb? function
function Server.SavePlayer(xPlayer, cb)
    local queries = {
        {
    ...
    MySQL.transaction(queries, function(success)
        ESX.Print(success and 'info' or 'error', (success and 'Saved player ^5`%s`^7' or 'Error in saving player ^5`%s`^7'):format(xPlayer.name))
...
---@param cb? function
function Server.SavePlayers(cb)
    ...
    MySQL.transaction(queries, function(success)
        ESX.Print(success and 'info' or 'error', (success and 'Saved ^5%s^7 %s over ^5%s^7 ms' or 'Failed to save ^5%s^7 %s over ^5%s^7 ms'):format(playerCounts, playerCounts > 1 and 'players' or 'player', ESX.Math.Round((os.time() - startTime) / 1000000, 2)))
...
---@return boolean
function Server.IsPlayerAdmin(playerId)
...
---@param playerId integer | number | string
---@return string
function Server.GetPlayerAdminGroup(playerId)
    playerId = tostring(playerId)
    local group = Server.DefaultGroup

    for i = 1, #Config.AdminGroups do -- start from the highest perm admin group
        local groupName = Config.AdminGroups[i]

        if IsPlayerAceAllowed(playerId, Server.Groups[groupName].principal) then
            group = groupName
            break
        end
    end
