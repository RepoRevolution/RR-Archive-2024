...
lib.callback.register('ox_inventory:openShop', function(source, data)
	...
	if data then
		...
		for _, slot in pairs(shop.items) do
			if not slot.currency or slot.currency == 'money' or slot.currency == 'bank' then
				slot.price = GetResourceState('rr_inflation') == 'started' and exports['rr_inflation']:getPrice(slot.base_price) or slot.base_price
			end
		end
