...
local function consoleLog(level, ...)
    ...
    table.insert(formattedArgs,
        (
            ...
            or level == 3 and '[^6WARNING^7]' or '[^4ERROR^7]'
        )
    )
...
if not IsDuplicityVersion() then
    ...
    function DISCORD_LOG(url, params)
        ...
        PerformHttpRequest(url, function(status, response, _, errorData)
            LOG(2, 'Response code:', tostring(status))
            LOG(2, 'Response:', tostring(response))
            LOG(2, 'Error:', tostring(errorData))
...
if GetResourceState('es_extended') == 'started' then
    success, message = lib.checkDependency('es_extended', '1.10.0')
    if not success then return LOG(4, message) end

    FRAMEWORK = 'ESX'
    ESX = exports["es_extended"]:getSharedObject()
elseif GetResourceState('qb-core') == 'started' then
    success, message = lib.checkDependency('qb-core', '1.2.0')
    if not success then return LOG(4, message) end
...
if not FRAMEWORK then return LOG(4, ('`%s` needs framework resource to run (ESX/QBCore)'):format(RESOURCE)) end
